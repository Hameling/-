...
class UpList(generics.ListAPIView):
...
class Upload(APIView):
    ...
    def post(self, request, *args, **kwargs):
        file_ex = str(request.data['fileex'])
        input_contentid = request.data["contentid"]

        file_serializer = UpSerializer(data=request.data)
        if file_serializer.is_valid():
            file_serializer.save()
            dir_name = str(input_contentid)
            filemove.file_movedir(file_ex,dir_name)
